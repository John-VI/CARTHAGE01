...
clk::window::window(const char name[], int width, int height,
    ...
    : color(color), win(nullptr, &SDL_DestroyWindow),
  ...
  if (renderer == nullptr)
    throw std::runtime_error(
        std::string({"Failed to initialize SDL renderer: ", SDL_GetError()}));
  ren.reset(renderer);
  setcolor(color);
}
...
SDL_Window *clk::window::getSDL_Window() { return win.get(); }

SDL_Renderer *clk::window::getSDL_Renderer() { return ren.get(); }
